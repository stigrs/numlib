 ...
 * http://www.ccl.net/cca/software/SOURCES/C/kinetics2/index.shtml and
 * modified by
 * Heng Li <lh3lh3@gmail.com>. Heng merged several C files into one and added a
 * simpler interface.
 * [Available here](http://lh3lh3.users.sourceforge.net/download/lsoda.c)
 *
...
Numlib::Lsoda::Lsoda(Lsoda_sys_t f, double atol, double rtol) : fsys(f)
{
    // Initialize arrays.
    mord = {{12, 5}};
    sm1 = {{0., 0.5, 0.575, 0.55, 0.45, 0.35, 0.25, 0.2, 0.15, 0.1, 0.075, 0.05,
            0.025}};
    el = {{0}};
    cm1 = {{0}};
    cm2 = {{0}};
...
bool Numlib::Lsoda::abs_compare(double a, double b)
{
    return (std::abs(a) < std::abs(b));
}

/* Purpose : Find largest component of double vector dx */
std::size_t Numlib::Lsoda::idamax1(const std::vector<double>& dx,
                                   const std::size_t nn,
                                   const std::size_t offset = 0)
{
    ...
    for (std::size_t i = 1; i <= nn; i++) {
        v = static_cast<std::size_t>(std::abs(dx[i + offset]));
        if (v > vmax) {
            vmax = v;
            idmax = i;
        }
    ...
    // Numlib::Lsoda::abs_compare) - dx.begin() - offset;
}

/* Purpose : scalar vector multiplication
   dx = da * dx
*/
void Numlib::Lsoda::dscal1(const double da,
...
double Numlib::Lsoda::ddot1(const std::vector<double>& a,
                            const std::vector<double>& b,
                            const std::size_t nn,
                            const std::size_t offsetA = 0,
                            const std::size_t offsetB = 0)
{
    double sum = 0.0;
    for (std::size_t i = 1; i <= nn; i++)
...
void Numlib::Lsoda::daxpy1(const double da,
                           const std::vector<double>& dx,
                           std::vector<double>& dy,
                           const std::size_t nn,
                           const std::size_t offsetX = 0,
                           const std::size_t offsetY = 0)
{
...
void Numlib::Lsoda::dgesl(const std::vector<std::vector<double>>& a,
                          const std::size_t nn,
...
void Numlib::Lsoda::dgefa(std::vector<std::vector<double>>& a,
                          const std::size_t nn,
                          std::vector<int>& ipvt_,
                          std::size_t* const info)
{
    ...
    *info = 0;
    for (k = 1; k <= nn - 1; k++) {
        ...
        */
        j = idamax1(a[k], nn - k + 1, k - 1) + k - 1;
        ipvt_[k] = static_cast<int>(j);
        /*
        ...
        */
        for (i = k + 1; i <= nn; i++) {
            ...
            daxpy1(t, a[k], a[i], nn - k, k, k);
        }
    } /* end k-loop  */

    ipvt_[nn] = static_cast<int>(nn);
    if (a[nn][nn] == 0.0)
        *info = nn;
}

/* Terminate lsoda due to illegal input. */
void Numlib::Lsoda::terminate(int* istate)
{
    if (illin == 5)
        std::cerr
...
void Numlib::Lsoda::terminate2(std::vector<double>& y, double* t)
{
...
void Numlib::Lsoda::successreturn(std::vector<double>& y,
                                  double* t,
                                  int itask,
                                  int ihit,
                                  double tcrit,
...
c-----------------------------------------------------------------------
*/
void Numlib::Lsoda::lsoda(Lsoda_sys_t f,
                          ...
                          void* _data)
{
    ...
    */
    if (*istate == 1) {
        ...
        */
        ...
        nyh = n;
        lenyh = static_cast<int>(1 + std::max(mxordn, mxords));
    ...
    */
    if (*istate == 1) {
        ...
        /* Load the initial value std::vector in yh_.  */
        for (std::size_t i = 1; i <= n; i++)
            yh_[1][i] = y[i];
    ...
    while (1) {
        if (*istate != 1 || nst != 0) {
            if ((nst - nslast) >= mxstep) {
                std::cerr << "[lsoda] " << mxstep
                          << " steps taken before reaching tout" << std::endl;
                *istate = -1;
                terminate2(y, t);
                return;
            }
        ...
        if (kflag == 0) {
            /*
               Block f.
               The following block handles the case of a successful return from
               the core integrator ( kflag = 0 ). If a method switch was just
               made, record tsw, reset maxord, set jstart to -1 to signal stoda
               to complete the switch, and do extra printing of data if ixpr
               = 1. Then, in any case, check for stop conditions.
            */
            init = 1;
            if (meth_ != mused) {
                tsw = tn_;
                ...
                if (ixpr) {
                    if (meth_ == 2)
                        std::cerr << "[lsoda] a switch to the stiff method has "
                                     "occurred "
                                  << std::endl;
                    if (meth_ == 1)
                        std::cerr
                            << "[lsoda] a switch to the nonstiff method has "
                               "occurred"
                            << std::endl;
                }
            } /* end if ( meth_ != mused )   */
...
void Numlib::Lsoda::stoda(const std::size_t neq,
                          std::vector<double>& y,
                          Lsoda_sys_t f,
                          void* _data)
{
        ...
        */
        ...
        if (dsm <= 1.0) {
            /*
               ...
               consider changing h_ if ialth = 1.  Otherwise decrease ialth
               by 1. If ialth is then 1 and nq < maxord, then acor is saved for
               use in a possible order increase on the next step.
               If a change in h_ is considered, an increase or decrease in order
               by one is considered also.  A change in h_ is made only if it is
...
void Numlib::Lsoda::ewset(const std::vector<double>& ycur)
{
    switch (itol_) {
    case 1:
        for (std::size_t i = 1; i <= n; i++)
...
void Numlib::Lsoda::intdy(double t, int k, std::vector<double>& dky, int* iflag)
{
    ...
    *iflag = 0;
    ...
    s = (t - tn_) / h_;
    ic = 1;
    for (std::size_t jj = l - k; jj <= nq; jj++)
        ic *= static_cast<int>(jj);
    c = (double) ic;
    for (std::size_t i = 1; i <= n; i++)
        dky[i] = c * yh_[l][i];

    for (std::size_t j = nq - 1; j >= k; j--) {
        jp1 = static_cast<int>(j + 1);
        ic = 1;
        for (std::size_t jj = jp1 - k; jj <= j; jj++)
            ic *= static_cast<int>(jj);
        c = (double) ic;
...
void Numlib::Lsoda::cfode(int meth__)
{
    int i;
    int nq_;
    int nqm1;
...
void Numlib::Lsoda::scaleh(double* rh, double* pdh)
{
    double r;
    /*
...
void Numlib::Lsoda::prja(const std::size_t neq,
                         std::vector<double>& y,
                         Lsoda_sys_t f,
                         void* _data)
{
    ...
    */
    if (miter != 2) {
        std::cerr << "[prja] miter != 2\n";
        return;
    }
    if (miter == 2) {
        fac = vmnorm(n, savf, ewt);
        r0 = 1000.0 * fabs(h_) * ETA * ((double) n) * fac;
        if (r0 == 0.0)
            r0 = 1.0;
        for (j = 1; j <= n; j++) {
            yj = y[j];
        ...
        */
        pdnorm = fnorm(static_cast<int>(n), wm_, ewt) / fabs(hl0);
        /*
           Add identity matrix.
        */
        for (i = 1; i <= n; i++)
...
*/
double Numlib::Lsoda::vmnorm(const std::size_t nn,
...
double Numlib::Lsoda::fnorm(int nn,
...

{
    ...
    for (std::size_t i = 1; i <= (std::size_t) nn; i++) {
        sum = 0.0;
        for (std::size_t j = 1; j <= (std::size_t) nn; j++)
            sum += fabs(a[i][j]) / w[j];
...
*/
void Numlib::Lsoda::correction(const std::size_t neq,
                               std::vector<double>& y,
                               Lsoda_sys_t f,
                               std::size_t* corflag,
                               double pnorm,
...
void Numlib::Lsoda::corfailure(double* told,
                               double* rh,
                               std::size_t* ncf,
                               std::size_t* corflag)
{
    ...
    rmax = 2.0;
    tn_ = *told;
    for (std::size_t j = nq; j >= 1; j--)
        for (std::size_t i1 = j; i1 <= nq; i1++)
            for (std::size_t i = 1; i <= n; i++)
                yh_[i1][i] -= yh_[i1 + 1][i];
...
/*
   ...
   y = the right-hand side vector on input, and the solution vector
       on output.
*/
void Numlib::Lsoda::solsy(std::vector<double>& y)
{
    iersl = 0;
    if (miter != 2) {
        std::cerr << "solsy -- miter != 2\n";
        return;
    }
    if (miter == 2)
...
void Numlib::Lsoda::methodswitch(double dsm,
                                 double pnorm,
                                 double* pdh,
                                 double* rh)
{
    ...
    if (meth_ == 1) {
        ...
        if (dsm <= (100.0 * pnorm * ETA) || pdest == 0.0) {
            if (irflag == 0)
                return;
            rh2 = 2.0;
            nqm2 = static_cast<int>(std::min(nq, mxords));
        }
        else {
            exsm = 1.0 / (double) l;
            ...
            *pdh = pdlast * fabs(h_);
            ...
            if (nq > mxords) {
                nqm2 = static_cast<int>(mxords);
                lm2 = static_cast<int>(mxords + 1);
                exm2 = 1.0 / (double) lm2;
                lm2p1 = lm2 + 1;
    ...
    */
    exsm = 1.0 / (double) l;
    if (mxordn < nq) {
        nqm1 = static_cast<int>(mxordn);
        lm1 = static_cast<int>(mxordn + 1);
        exm1 = 1.0 / (double) lm1;
        lm1p1 = lm1 + 1;
...
*/
void Numlib::Lsoda::orderswitch(
    double* rhup, double dsm, double* pdh, double* rh, std::size_t* orderflag)
{
    std::size_t newq = 0;
 ...
 */
/* ----------------------------------------------------------------------------*/
void Numlib::Lsoda::integrate(Lsoda_sys_t f,
                              const std::size_t neq,
                              std::vector<double>& y,
                              ...
                              double atol)
{
    ...
    // Fill-in values.
    for (std::size_t i = 1; i <= neq; i++)
        yout[i] = y[i - 1];

    lsoda(f, neq, yout, t, tout, itask, istate, iopt, jt, iworks, rworks,
          _data);
}
