...
/* First approximation to the integral. */
    ...
    result = cquadpack_G_K15I(f,boun,inf,0.0,1.0,abserr,&defabs,&resabs, user_data);
...
/* Main loop. */
    for (last = 1; last <= limit; last++) {
        ...
        b2 = blist[maxerr];
        erlast = errmax;
        area1 = cquadpack_G_K15I(f,boun,inf,a1,b1,&error1,&resabs,&defab1, user_data);
        area2 = cquadpack_G_K15I(f,boun,inf,a2,b2,&error2,&resabs,&defab2, user_data);
...
/* Call dqsort to maintain the descending ordering in the list of error
    ...
    error estimate (to be bisected next). */
        ...
        cquadpack_dqsort(limit,last,&maxerr,&errmax,elist,iord,&nrmax);
...
_60:
        numrl2++;
        rlist2[numrl2] = area;
        reseps=cquadpack_dqext(&numrl2,rlist2,&abseps,res3la,&nres);
        ktmin++;
